...
// A helper function that returns the next number in the fibonacci sequence given the current fibonacci sequence
function nextFib(fibonacciSequence) {
    var sequenceLength = fibonacciSequence.length;

    return fibonacciSequence[sequenceLength - 1] + fibonacciSequence[sequenceLength - 2];
}

function nthFibonacci(n) {
    // Initialize our sequence so we have at minimal numbers needed to calculate the next number in sequence
    var fibonacciSequence = [0, 1];

    // Continue to add more numbers to our sequence until we have at least n number of numbers in our sequence 
    while (fibonacciSequence.length < n) {
        fibonacciSequence.push(nextFib(fibonacciSequence));
    }
